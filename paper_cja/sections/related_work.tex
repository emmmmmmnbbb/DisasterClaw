...
具身智能需要知道何时观察不够。本文不把端到端成功率为零解释为``机制无效''：若上游从未产生损伤证据，重观测控制器根本没有被执行。
这是实验设计问题，也是本文把机制研究从导航链解耦的原因。

\paragraph{MLLM 驱动的具身搜救基准。}
与本文同期，ESARBench 把 MLLM 驱动的无人机具身智能体在搜救任务上标准化为基准：四个 UE5+AirSim、真实 GIS 重建场景、600 个搜救任务，指标为成功率/时间加权成功率/线索发现/救援分\cite{zhang2026esarbench}。
其瓶颈诊断落在空间记忆与搜索效率--飞行安全权衡。本文与其互补而非重叠：ESARBench 测搜救导航与受害者定位，本文测损伤评估与预算受限重观测。
