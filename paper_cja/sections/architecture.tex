...
\paragraph{任务规划模式。}
操作员在控制台输入一条自然语言任务（例如``飞到选点上空 30\,m 悬停并做受灾检测''）。
...
系统进入感知--决策--执行闭环：每步按当前经纬高裁剪机载视场，运行检测/分割/双时相损伤分类，
可选地由复检控制器发出下降--居中，再由导航器输出相对位移、悬停或停止。
导航器可切换为关键词贪心基线（套件 B0）或分层语义规划模块 HSPM（B1--B3 与 oracle 阶梯默认，见 \secref{sec:hspm}）。

\paragraph{底层动作。}
两种模式最终都落到同一动作表：
\texttt{fly\_to\_geo}（飞到经纬高，必要时对齐最近的灾后瓦片）、
\texttt{fly\_relative}（北--东--天相对位移）、
\texttt{hover}、
\texttt{detect\_disaster}（裁视场并跑感知栈）、
\texttt{mark\_target}、
\texttt{report\_observation}。
语言导航另把 \texttt{stop} 作为回合终止符。
复检不是第 7 种独立动作枚举，而是在闭环内插入的 \texttt{fly\_relative}（先水平居中、再下降 $10\,\mathrm{m}$）。

\paragraph{感知栈。}
机载视场半径随高度线性变化（默认 $\mathrm{radius}= \mathrm{clip}(2\cdot h,\,20,\,300)\,\mathrm{m}$），再按瓦片地面采样距离裁成至少 $256\times256$ 的正射块。
块上并行：建筑提议（默认仅在训练事件灾前瓦片上训练的 ResNet34 U-Net，同切分 YOLO 作对照）、场景分割（SegFormer）、双时相损伤分类，以及进程内加载的 Qwen2.5-VL-7B 开放词汇指认与灾情摘要。
混合指认路径为检测器类别命中优先、视觉语言模型兜底。
禁止把定位 Dice 或提议召回写成导航或判断增益。

\subsection{地理配准仿真世界}
仿真世界把 xBD 灾后（POST）瓦片按地理足迹叠到真实底图上，像素--经纬度由瓦片仿射给出，原生地面采样距离约 $0.5\,\mathrm{m/px}$。
...
巡航高度 $30\,\mathrm{m}$，高度下限 $10\,\mathrm{m}$，不含风扰、惯性、避障与电池消耗。
视场随高度收缩；离线机制实验里再把高度写成有效 GSD 阶梯（\secref{sec:gsd}）。
...
\item \textbf{点选巡检。} 在疑似受损建筑上发 Inspect：自动生成任务规划序列，飞到 30\,m 悬停后跑 \texttt{detect\_disaster}。底栏可在``UAV 视场 / YOLO 标注 / SegFormer''三页签间切换，并显示风险等级、受损/完好/车辆计数与可选的视觉语言模型摘要。
