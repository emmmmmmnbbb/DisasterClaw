...
它是后续机制实验的运行载体，不是对导航成功率或重观测增益的度量声明；
...
\label{sec:dualmode}
系统由事件驱动的服务端与响应式控制台构成，面向单架无人机、正射地理鸟瞰。
编排器提供两种互补模式，动作空间显式、有限，不是开放的工具调用循环：大语言模型只负责规划与指认，状态机在确定性执行器中闭合。
...
可选地由重观测控制器发出下降--居中，再由导航器输出相对位移、悬停或停止。
导航器可切换为关键词贪心基线（套件 B0）或分层语义规划模块 HSPM（B1--B3 与 oracle 阶梯默认，见 \secref{sec:hspm}）。

\paragraph{底层动作。}
两种模式最终都落到同一组六类原子动作：飞到经纬高、相对位移、悬停、检测、标记与报告。
语言导航另把停止作为回合终止符。
重观测不是第 7 种独立动作枚举，而是在闭环内插入的相对位移（先水平居中、再按视场阶梯下降一档）。
...
灾前通道保持原生 GSD，不随高度降质；建筑提议在灾前整瓦片上以原生分辨率运行并按瓦片缓存，再把框仿射映射进当前视场，跨瓦片重叠以交并比 $0.55$ 做非极大值抑制。
在线闭环默认已切换为冠军方案检测后端（感知底座可分派）：灾前/灾后窗口喂双时相损伤分类，检测框按 ROI 过滤后再送下游。
事件不相交训练的编码器--解码器定位网络与轻量双时相头保留为可回退的对照基线。
同切分的单阶段检测器与场景分割、本地离线推理的开放词汇指认与灾情摘要仍作为并列模块。
...
\label{sec:console}
前端是实时监控/控制台，不是评测脚本的附属页面。操作员可以完成下列闭环，全部经事件通道推送世界状态、感知结果与思维链。
...
\item \textbf{点选巡检。} 在疑似受损建筑上发 Inspect：自动生成任务规划序列，飞到巡航高度悬停后跑 \texttt{detect\_disaster}。底栏可在``机载视场 / 检测标注 / 场景分割''三页签间切换，并显示风险等级、受损/完好/车辆计数与可选的视觉语言模型摘要。
\item \textbf{语言目标搜索。} 在 VLN 框下发一句话指令，无人机逐步感知并自主飞向目标；底栏显示当前技能、是否命中、距离、语义栅格统计（已探索格、地标、候选目标）以及重观测不确定性条。
...
\platformshot{vln_closedloop}{语言导航进行中：地图轨迹、当前指令、技能与命中状态、语义栅格统计及重观测不确定性。}{fig:ui-vln}
