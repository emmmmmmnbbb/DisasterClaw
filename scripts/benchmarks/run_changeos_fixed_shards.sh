#!/usr/bin/env bash
# Dynamically schedule the corrected ChangeOS Agent-VQA run without sharing
# perception artifacts between shards.  A shard starts only when a GPU has the
# requested free-memory headroom; failed shards resume at durable JSONL rows.
set -uo pipefail
...
RUN_PREFIX=runs/benchmarks/changeos_binary/full_fixed_20260913
LOG_PREFIX=/tmp/bench_fixed_20260913
...
validate_shard() {
  local shard=$1
  "$PY" -c 'import json,sys; from pathlib import Path; p=Path(sys.argv[1]); r=p/"results.json"; e=p/"episodes.jsonl"; d=json.loads(r.read_text()) if r.is_file() else {}; lines=sum(1 for _ in e.open()) if e.is_file() else 0; raise SystemExit(0 if d.get("valid_for_analysis") is True and lines==320 else 1)' \
...
while (( ${#pending[@]} > 0 || running_count > 0 )); do
  ...
  for shard in "${!pid_by_shard[@]}"; do
    ...
    if (( rc == 0 )) && validate_shard "$shard"; then
      echo "[scheduler] $(date '+%F %T') shard $shard complete and validated (320 rows)"
  ...
  if (( ${#pending[@]} > 0 )); then
    mapfile -t gpu_rows < <(nvidia-smi --query-gpu=index,memory.free --format=csv,noheader,nounits 2>/dev/null | tr -d ' ' | sort -t, -k2,2nr)
    remaining=()
