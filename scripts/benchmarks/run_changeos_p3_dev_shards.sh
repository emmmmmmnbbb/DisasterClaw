#!/usr/bin/env bash
# Schedule the P3 matched action-ablation development run.
set -uo pipefail
...
CONFIGS=${CONFIGS:-T1_TASK,AB_NOOP,AB_CENTER,AB_DESCEND,AB_FULL,AB_WIDE,D0_RULE,V2_STATE_VLM,A0_VLM}
RUN_PREFIX=${RUN_PREFIX:-runs/benchmarks/changeos_binary/p3_dev_v1_20260914}
LOG_PREFIX=${LOG_PREFIX:-/tmp/bench_p3_dev_v1_20260914}
...
validate_shard() {
  ...
  "$PY" -c 'import json,sys; from pathlib import Path; p=Path(sys.argv[1]); r=p/"results.json"; e=p/"episodes.jsonl"; rows=[json.loads(x) for x in e.read_text().splitlines() if x.strip()] if e.is_file() else []; expected=int(sys.argv[2]); infra=sum(1 for x in rows if not x.get("ok") and x.get("reason_code") != "out_of_coverage"); raise SystemExit(0 if r.is_file() and len(rows)==expected and infra==0 else 1)' \
    "$out" "$((ROWS_PER_SHARD * $(awk -F, '{print NF}' <<<"$CONFIGS")))" || return 1
  "$PY" scripts/benchmarks/audit_ablation_schedule.py "$out" \
    --out "$out/schedule_audit.json"
}

launch_shard() {
  local shard=$1 gpu=$2 out=${RUN_PREFIX}_shard${shard}of4 log=${LOG_PREFIX}_shard${shard}.log
  local lock=/tmp/disasterclaw_changeos_p3_shard${shard}.lock
  ...
  PERCEPTION_DEVICE="cuda:$gpu" VLM_LOCAL_DEVICE="cuda:$gpu" PERCEPTION_OUTPUT_DIR="backend/outputs/uav_view_p3_shard${shard}" \
    flock -n "$lock" "$PY" -u scripts/benchmarks/bench_agent_vqa.py --testset "$TEST" --configs "$CONFIGS" \
      --seed 42 --generation-seed 42000 --generation-repeat 0 --tag changeos_binary_p3 \
      --shard "$shard/4" --out-dir "$out" --resume --allow-crash-resume >>"$log" 2>&1 &
...
while (( ${#pending[@]} > 0 || running_count > 0 )); do
  for shard in "${!pid_by_shard[@]}"; do
    ...
    elif (( ${attempts_by_shard[$shard]:-0} < MAX_ATTEMPTS )); then pending+=("$shard")
    else failed=1; fi
  done
  ...
  for shard in "${pending[@]}"; do
    ...
    if [[ -n "$selected" ]] && launch_shard "$shard" "$selected"; then busy_gpu[$selected]=1; running_count=$((running_count+1)); else remaining+=("$shard"); fi
  done
  pending=("${remaining[@]}")
